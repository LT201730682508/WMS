...
class Ware_In_Record_Dialog(context: Context, var showSure: Show_Sure, var result: Ware_In_Record_Model.Ware_Record, var userLogin: Login_fragment.user_Login) : Dialog(context, R.style.CustomDialog) {
    lateinit var search_text: EditText
    lateinit var search_img: ImageView
    lateinit var record_ry: RecyclerView
    lateinit var arrayAdapter: Ware_In_Ware_Adapter
        ...
        Join_Warehouse_Model.getData(object : Join_Warehouse_Model.Show {
            override fun show(wares: ArrayList<All_Warehouse_Model.Warehouse>) {
                if (wares.size != 0) {
                    ...
                    mList.addAll(wares)
                    arrayAdapter= Ware_In_Ware_Adapter(mList,result,showSure,userLogin,this@Ware_In_Record_Dialog)
